...
def get_text_messages(message):
    ...
    if message.text == '🔙 Главное меню':
        ...
        bot.send_message(message.from_user.id, 'Сердце компании - наши квалифицированные высококлассные сотрудники,'
                                               'с опытом работы в сфере систем вентиляции и кондиционирования, более 15 лет.\n'
                                               'Это увлеченные своим делом люди с креативным мышлением, что позволяет '
                                               'подходить комплексно к решению поставленных задач.\n'
                                               'Мы всегда готовы помочь создать комфорт нашим клиентам.')
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=1)
        btn1 = types.KeyboardButton('🔙 Главное меню')
        markup.add(btn1)
        keyboard = types.InlineKeyboardMarkup()
        button = types.InlineKeyboardButton(text="Связаться с нами", url="https://t.me/@MOSSPLIT_INFO")
        keyboard.add(button)
        bot.send_contact(message.from_user.id, '+79999118533', 'MOSSPLIT' 'INFO', reply_markup=keyboard)
        # bot.send_message(message.from_user.id, 'Мы в Instagram\n' + setting.INSTAGRAM, disable_web_page_preview=True)
        bot.send_message(message.from_user.id, 'Мы в ВК\n' + setting.VK, disable_web_page_preview=True,
                         reply_markup=markup)
        time.sleep(0.5)

    elif message.text == '📝 Подписаться на канал':
        keyboard = types.InlineKeyboardMarkup()
        button = types.InlineKeyboardButton(text="Подписаться на канал", url="https://t.me/mossplit")
        keyboard.add(button)
